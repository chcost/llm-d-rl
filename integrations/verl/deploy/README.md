...
- [`epp-config-burst.yaml`](../common/deploy/epp-config-burst.yaml) - EPP scorer config (standard / burst routing). Parser defaults to `vllmhttp-parser`.
- [`deploy/epp-config-pd.yaml`](epp-config-pd.yaml) - EPP scorer config (PD disaggregated)
- [`envoy.yaml`](../common/deploy/envoy.yaml) - Envoy proxy config (llm-d serving mode only)
...
| `rollout.custom.epp_config_file` | yes | - | Path to the EPP YAML config (plugin list, scorers). Start from `common/deploy/epp-config-burst.yaml`. |
...
| `rollout.custom.epp_config_file` | yes | - | Path to the EPP YAML config. Start from `common/deploy/epp-config-burst.yaml`. |
...
| `rollout.custom.envoy_config` | yes | - | Path to the Envoy config YAML. Start from `common/deploy/envoy.yaml`. |
